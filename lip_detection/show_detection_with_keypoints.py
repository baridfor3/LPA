...
LIP_MODEL_PATH = os.path.join(ROOT_DIR, "mask_rcnn_lip_0123.h5")
class InferenceConfig(first_model_lip.LipConfig):
    GPU_COUNT = 0
...
model_path = os.path.join(ROOT_DIR, "mask_rcnn_lip_0123.h5")
...
cap = cv2.VideoCapture('/Users/barid/Documents/workspace/batch_data/5535415699068794046/00001.mp4')

count = 1
while(count < 13):
    # get a frame
    ret, frame = cap.read()
    # frame = cv2.imread("aquib/aquib (%d).jpg" % count)


    "BGR->RGB"
    # height, width, layers = frame.shape
    # y = int(height/2)
    # frame = frame[y:y + height, :width]
    ...
    cv2.imwrite("frame%d.jpg" % count, result_image)
    count += 1
